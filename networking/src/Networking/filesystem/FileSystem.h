...
#include <Networking/utilities/containers/Memory.h>
...
namespace EM {
	...
	public:
		...
		// Copy directory
		static void CopyDirectory(const char* origin, const char* destination);
		...
		*/
		...
		// Directly read all bytes from file and return memory handler
		static Memory ReadAll(const char* file);

		// Directly write all bytes to file
		static void WriteAll(const char* file, const sbyte* memblock, size_t size);
